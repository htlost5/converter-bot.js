...
client.on(Events.MessageCreate, sourceMessage => {
    ...
    if (typeof message.content === 'string' && message.content.startsWith("!")) {
        ...
        if (message.content.match(/start/) && status === false) {
            ...
            if(!match) {
                generalWebhook.send({
                    content: "error: startコマンドの形式が不正です。" + '\n' + "!start: n(２以上３６以下の数字)という形式で入力してください。",
                    username: 'vscode',
                    avatarURL: avatarURLs.vscode
                });
                return;
            ...
            generalWebhook.send({
                content: `${n}進数への変換を開始します` + '\n' + `変換中のメッセージは、${channelURLs.save}に保存されます。` + '\n' +'また、!translateコマンドを使うことで直前の文を翻訳できます。',
                username: 'vscode',
                avatarURL: avatarURLs.vscode
            });
            return
        ...
        // エラーハンドリング（ステータスとコマンドの矛盾）
            ...
            generalWebhook.send({
                content: 'すでに実行されているか、形式が間違っています。',
                username: 'vscode',
                avatarURL: avatarURLs.vscode
            });
            return;
